...
f = 1.16;
R_b_values = [1e9 2.5e9];         
t_d_values = [10e-9 1e-6 10e-6]; 
color = {'b','r'};
lineStyle = {'-','--','-.'};
R_all = zeros(length(R_b_values), length(t_d_values), length(L));
figure;
hold on;
for n = 1:length(R_b_values)
    R_b = R_b_values(n);
    for m = 1:length(t_d_values)
        t_d = t_d_values(m);
...
R(R < 1e-10) = NaN; 
R_all(n,m,:) = R;
   semilogy(L,R,'Color',color{n},'LineStyle',lineStyle{m},'LineWidth',2);
   set(gca, 'YScale', 'log');
    end
end
...
grid on;
legend({'R_b = 10^9, t_d = 10 ns','R_b = 10^9, t_d = 1 \mus','R_b = 10^9, t_d = 10 \mus','R_b = 2.5\times 10^9, t_d = 10 ns','R_b = 2.5\times 10^9, t_d = 1 \mus','R_b = 2.5\times 10^9, t_d = 10 \mus'},'Location','southwest');
xlim([0 230]);
ylim([1e-2 1e8]);
box on;
...
hold off;
%% Inset Plot
ax_inset = axes('Position',[0.55 0.17 0.30 0.25]);
hold(ax_inset,'on');
for n = 1:length(R_b_values)
    for m = 1:length(t_d_values)
        R_inset = squeeze(R_all(n,m,:));
 semilogy(ax_inset,L,R_inset,'Color',color{n},'LineStyle',lineStyle{m},'LineWidth',1.5);

    end
end
%% Inset limits
xlim(ax_inset,[221 222.2]);
ylim(ax_inset,[5e-2 1e2]);
