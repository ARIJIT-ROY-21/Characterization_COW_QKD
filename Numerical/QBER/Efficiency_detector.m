...
eta = 0.1:0.005:1; % Efficiency of detector
...
figure;
yyaxis left
plot(eta,V,'b-','LineWidth',3);
ylabel('Visibility (V)' , 'FontSize', 13,'FontWeight', 'bold');
ylim([0.99895 0.99995]);
yyaxis right
plot(eta,Q,'r-','LineWidth',3);
ylabel('QBER(%)' , 'FontSize', 13,'FontWeight', 'bold');
ylim([0.001 0.055]);
title('Efficiency of SPD vs QBER - COW QKD');
xlabel('Efficiency of SPD' , 'FontSize', 13,'FontWeight', 'bold');
xlim([0.1 1]); 
grid on;
box on;
