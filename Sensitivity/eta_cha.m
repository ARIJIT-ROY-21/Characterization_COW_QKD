...
d_t = 1e-9; % 1/Bit rate
R_b = 1e9; % Bit Rate
L = 80:-6.64:73.36; % Length of Fiber
...
% Sensitivity Analysis of QBER under various eta_channel
dQdeta_channel = gradient(Q,eta_channel);
S_eta_channel_Q = (eta_channel./Q).*dQdeta_channel;
S1_eta_channel_Q = S_eta_channel_Q(1);  
S2_eta_channel_Q = S_eta_channel_Q(2);      
percentage_change_eta_channel_Q = abs((S2_eta_channel_Q - S1_eta_channel_Q)/S1_eta_channel_Q)*100;
fprintf('Change in S_eta_channel_Q = %.2f %%\n', percentage_change_eta_channel_Q);
%% Sensitivity Analysis of SKR in Central Difference Method
% Sensitivity Analysis of SKR under various eta_channel
dRdeta_channel = gradient(R,eta_channel);
S_eta_channel_R = (eta_channel./R).*dRdeta_channel;
S1_eta_channel_R = S_eta_channel_R(1);  
S2_eta_channel_R = S_eta_channel_R(2);      
percentage_change_eta_channel_R = abs((S2_eta_channel_R - S1_eta_channel_R)/S1_eta_channel_R)*100;
fprintf('Change in S_eta_channel_R = %.2f %%\n', percentage_change_eta_channel_R);
