clc; clear;
% parameters
mu = 0.5:0.25:0.75; % Mean Number of Photons/Pulse
...
fprintf('mu = %.5f\n', mu );
% Sensitivity Analysis of QBER under various MNP
dQdmu = gradient(Q,mu);
S_mu_Q = (mu./Q).*dQdmu;
S1_mu_Q = S_mu_Q(1);  
S2_mu_Q = S_mu_Q(2);      
percentage_change_mu_Q = abs((S2_mu_Q - S1_mu_Q)/S1_mu_Q)*100;
fprintf('Change in S_mu_Q = %.2f %%\n', percentage_change_mu_Q);
%% Sensitivity Analysis of SKR in Central Difference Method
% Sensitivity Analysis of SKR under various MNP
dRdmu = gradient(R,mu);
S_mu_R = (mu./R).*dRdmu;
S1_mu_R = S_mu_R(1);  
S2_mu_R = S_mu_R(2);      
percentage_change_mu_R = abs((S2_mu_R - S1_mu_R)/S1_mu_R)*100;
fprintf('Change in S_mu_R = %.2f %%\n', percentage_change_mu_R);