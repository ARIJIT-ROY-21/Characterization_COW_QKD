...
fprintf('Bit Rate = %.10f\n hz', R_b );
% Sensitivity Analysis of QBER under various Bit Rate
dQdR_b = gradient(Q,R_b);
S_R_b_Q = (R_b./Q).*dQdR_b;
S1_R_b_Q = S_R_b_Q(1);  
S2_R_b_Q = S_R_b_Q(2);      
percentage_change_R_b_Q = abs((S2_R_b_Q - S1_R_b_Q)/S1_R_b_Q)*100;
fprintf('Change in S_R_b_Q = %.2f %%\n', percentage_change_R_b_Q);
%% Sensitivity Analysis of SKR in Central Difference Method
% Sensitivity Analysis of SKR under various Bit Rate values
dRdR_b = gradient(R,R_b);
S_R_b_R = (R_b./R).*dRdR_b;
S1_R_b_R = S_R_b_R(1);  
S2_R_b_R = S_R_b_R(2);      
percentage_change_R_b_R = abs((S2_R_b_R - S1_R_b_R)/S1_R_b_R)*100;
fprintf('Change in S_R_b_R = %.2f %%\n', percentage_change_R_b_R);