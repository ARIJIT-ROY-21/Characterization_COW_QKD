...
P_d = 2e-7:1e-7:3e-7; % Dark Cout Probability/Pulse in detector
...
fprintf('Dark Count Rate = %.10f\n hz', P_d );
% Sensitivity Analysis of QBER under various detector DCR
dQdP_d = gradient(Q,P_d);
S_P_d_Q = (P_d./Q).*dQdP_d;
S1_P_d_Q = S_P_d_Q(1);  
S2_P_d_Q = S_P_d_Q(2);      
percentage_change_P_d_Q = abs((S2_P_d_Q - S1_P_d_Q)/S1_P_d_Q)*100;
fprintf('Change in S_P_d_Q = %.2f %%\n', percentage_change_P_d_Q);
%% Sensitivity Analysis of SKR in Central Difference Method
% Sensitivity Analysis of SKR under various detector DCR
dRdP_d = gradient(R,P_d);
S_P_d_R = (P_d./R).*dRdP_d;
S1_P_d_R = S_P_d_R(1);  
S2_P_d_R = S_P_d_R(2);      
percentage_change_P_d_R = abs((S2_P_d_R - S1_P_d_R)/S1_P_d_R)*100;
fprintf('Change in S_P_d_R = %.2f %%\n', percentage_change_P_d_R);