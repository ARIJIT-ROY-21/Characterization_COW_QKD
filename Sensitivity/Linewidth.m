...
delta_f = 1e3:0.5e3:1.5e3; % Linewidth in Hz
d_t = 1e-9; % 1/Bit rate
R_b = 1e9; % Bit Rate
...
fprintf('Linewidth = %.5f\n', delta_f );
% Sensitivity Analysis of QBER under various LASER Linewidth
dQddelta_f = gradient(Q,delta_f);
S_delta_f_Q = (delta_f./Q).*dQddelta_f;
S1_delta_f_Q = S_delta_f_Q(1);  
S2_delta_f_Q = S_delta_f_Q(2);      
percentage_change_delta_f_Q = abs((S2_delta_f_Q - S1_delta_f_Q)/S1_delta_f_Q)*100;
fprintf('Change in S_delta_f_Q = %.2f %%\n', percentage_change_delta_f_Q);
%% Sensitivity Analysis of SKR in Central Difference Method
% Sensitivity Analysis of SKR under various LASER Linewidth
dRddelta_f = gradient(R,delta_f);
S_delta_f_R = (delta_f./R).*dRddelta_f;
S1_delta_f_R = S_delta_f_R(1);  
S2_delta_f_R = S_delta_f_R(2);      
percentage_change_delta_f_R = abs((S2_delta_f_R - S1_delta_f_R)/S1_delta_f_R)*100;
fprintf('Change in S_delta_f_R = %.2f %%\n', percentage_change_delta_f_R);