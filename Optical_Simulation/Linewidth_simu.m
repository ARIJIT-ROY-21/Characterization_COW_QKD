...
delta_f = 1e0:1e4:1e8; % Linewidth in Hz
d_t = 1e-9; % 1/Bit rate
...
Q = (1 - V).*100./2;
% Excel data
filename = 'Linewidth_simu.xlsx';
data = readtable(filename);
Linewidth_excel = data{:,1};
Q_excel  = data{:,5};
figure;
semilogx(delta_f, Q, 'r-', 'LineWidth', 3, 'DisplayName', 'Numerical Model');
hold on;
semilogx(Linewidth_excel, Q_excel, 'o', 'Color', [0.75 0 0.35], 'MarkerFaceColor', [0.75 0 0.35],'MarkerSize', 7, 'LineWidth', 1.5, 'DisplayName', 'Simulation Result');
hold off;
ylabel('QBER (%)', 'FontSize', 13,'FontWeight', 'bold');
xlabel('Linewidth of LASER (Hz)', 'FontSize', 13,'FontWeight', 'bold');
