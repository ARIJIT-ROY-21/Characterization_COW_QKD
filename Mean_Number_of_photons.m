clc; clear;
% parameters
mu = 0.1:0.001:1; % Mean Number of Photons/Pulse
...
Q_0 = (1 - V)./2;
Q = (1 - V).*100./2;
% Mean Number of Photons/Pulse Graph
figure;
yyaxis left
plot(mu,V,'b-','LineWidth',3);
ylabel('Visibility (V)' , 'FontSize', 13,'FontWeight', 'bold');
yyaxis right
plot(mu,Q,'r-','LineWidth',3);
ylabel('QBER(%)' , 'FontSize', 13,'FontWeight', 'bold');
title(' Mean Number of Photons/Pulse vs QBER - COW QKD');
xlabel('Mean Number of Photons/Pulse');
xlim([0.1 1]); 
grid on;
box on;
